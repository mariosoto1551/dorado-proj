...
/**
 * Schema de variables de entorno de audit-service (ADR-00 §8): el proceso NO
 * arranca si falta una requerida o tiene formato inválido. Sin URLs internas
 * salientes: audit no llama a ningún servicio — guarda los payloads tal cual.
 */
export class EnvSchema {
  ...
  // Clave pública RS256 (ADR-00 §3): audit valida el JWT en /audit/*.
  @IsString()
  @IsNotEmpty()
  ...
  // RabbitMQ (ADR-00 §5): esta fase consume los eventos administrativos y de
  // ciclo de vida (no publica nada).
  @Matches(/^amqps?:\/\/.+/, {
...
export function validarEnv(config: Record<string, unknown>): EnvSchema {
  ...
  if (detalles.length > 0) {
    throw new Error(
      `Variables de entorno inválidas en audit-service:\n- ${detalles.join('\n- ')}`
    );
  }
