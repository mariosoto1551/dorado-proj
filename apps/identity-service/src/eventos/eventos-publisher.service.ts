...
/**
 * Publicador de eventos de dominio de identity-service. Envuelve SIEMPRE el
...
export class EventosPublisherService {
  ...
  async publicar<T>(evento: EventoAPublicar<T>): Promise<void> {
    const envelope: EventEnvelope<T> = {
      ...
      // Propagado desde el request HTTP que originó el evento; si no hay
      // request asociado se genera uno nuevo (ADR-00 §5).
