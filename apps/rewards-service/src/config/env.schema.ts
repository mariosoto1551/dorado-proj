...
/**
 * Schema de variables de entorno de rewards-service (ADR-00 §8): el proceso
...
export class EnvSchema {
  ...
  // Clave pública RS256 (ADR-00 §3): rewards valida el JWT de usuario en
  // todas sus rutas /rewards/*. La privada NUNCA va acá (solo identity emite).
  @IsString()
  @IsNotEmpty()
  ...
  // RabbitMQ (ADR-00 §5): consume ZonaAlcanzada, publica RecompensaCanjeada.
  @Matches(/^amqps?:\/\/.+/, {
    message: 'RABBITMQ_URL debe ser una URL amqp(s)://',
  })
  RABBITMQ_URL!: string;

  // REST interno (ADR-00 §4): validación de grupos/usuarios (regla 3 de
  // CLAUDE.md — el cliente nunca decide el tenant).
  @Matches(/^https?:\/\/.+/, {
    message: 'IDENTITY_INTERNAL_URL debe ser una URL http(s)://',
  })
  IDENTITY_INTERNAL_URL!: string;

  // REST interno (ADR-00 §4): validar umbralZonaId al crear/editar Recompensa
  // y consultar el ResultadoSeccion para la elegibilidad de canje.
  @Matches(/^https?:\/\/.+/, {
...
export function validarEnv(config: Record<string, unknown>): EnvSchema {
  ...
  if (detalles.length > 0) {
    throw new Error(
      `Variables de entorno inválidas en rewards-service:\n- ${detalles.join('\n- ')}`
    );
  }
