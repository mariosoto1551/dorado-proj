...
| `minimo-de-repeticiones.e2e.ts` | fase-14 · ítem 25 | El **castigo proporcional** del cierre contra el ledger de scoring: 1 de 3 resta 2×, 0 de 3 resta 3×, y llegar al mínimo no resta nada. Incluye la **retro-compatibilidad** (mínimo 1 = una confirmación alcanza, como antes del ítem) y el 400 de un mínimo mayor que el máximo. Los unit tests verifican la fila que escribe el cierre; solo el stack real verifica que scoring reste lo que esa fila dice. |
| `etiquetas-catalogo.e2e.ts` | fase-14 · ítem 26 | Las **etiquetas del catálogo** contra el stack real: el ciclo etiquetar → filtrar → archivar → **recuperar con los mismos ítems** (los unit tests lo comprueban contra un array; acá contra Postgres con sus FK), el **aislamiento de tenant** —una etiqueta de otra organización da `ETIQUETA_INVALIDA` porque la extensión de Prisma la esconde, no por un `if` del service—, la publicación masiva **idempotente** (correrla dos veces da `SIN_ITEMS_PARA_CREAR` y la tienda queda con 2 productos, no 4), el `SOLO_EN_MODO_TIENDA` en modo DIRECTO, y que **el participante no reciba ninguna etiqueta por HTTP**, que es donde la decisión 3 se cumple o se filtra. |
