...
@Component({
  ...
  template: `
    <section class="mx-auto max-w-xl px-4 py-5">
      <h1 class="text-xl font-bold tracking-tight text-slate-900">Mi conducta</h1>
      <p class="mt-1 text-sm text-slate-500">Si te mandaste una macana, reportala acá. Suma honestidad. 🙌</p>
        ...
        <ul class="mt-5 space-y-2.5">
          @for (c of autoreportables(); track c.id) {
            <li class="flex items-center gap-3 rounded-2xl border-2 border-slate-100 bg-white p-4 shadow-sm">
              <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-red-50 text-red-500">
                <span class="h-5 w-5"><app-icono nombre="flag" /></span>
              </span>
              <div class="min-w-0 flex-1">
                <p class="font-semibold text-slate-900">{{ c.nombre }}</p>
                <p class="text-xs font-medium text-red-500">−{{ c.valorPuntos }} pts</p>
              </div>
              <button
                type="button"
                (click)="reportar(c)"
                [disabled]="procesando()"
                class="shrink-0 rounded-full border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 disabled:opacity-50"
              >
                Reportar
              </button>
            </li>
      ...
      @if (reportadas().length > 0) {
        <h2 class="mt-8 mb-2 text-sm font-bold text-slate-500 uppercase">Reportado hoy</h2>
        <ul class="space-y-1.5">
          @for (nombre of reportadas(); track $index) {
            <li class="rounded-lg bg-slate-100 px-3 py-2 text-sm text-slate-600">{{ nombre }}</li>
          }
        </ul>
      }
    </section>
