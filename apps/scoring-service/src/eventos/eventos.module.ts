 ...
 * (`scoring.dlq`, cuórum) donde terminan los mensajes que agotan reintentos —
 * para revisión manual, nunca descarte silencioso.
 *
 * Scoring consume (handlers @RabbitSubscribe en el módulo `consumo`) y
 * publica (ZonaAlcanzada / UsuarioDescalificado vía EventosPublisherService).
 */
@Global()
@Module({
  imports: [
