...
| Fase 13 — Piloto/deploy | COMPLETADA_CON_DESVIACIONES | 2026-07-24 | Infra-as-code **deploy-ready**: Dockerfile parametrizado + docker-compose.stack.yml, `infra/render.yaml` (Blueprint 9 servicios + Postgres), vercel.json de ambos frontends (+ environment.prod.ts/fileReplacements verificado), `.env.production.example`, `docs/runbook-deploy.md` y job **e2e en CI** (cierra deuda de Fase 12). Plataformas: Render + Vercel + CloudAMQP. **José confirmó estable el 2026-07-24** → se desbloquea Fase 14. Deuda residual (no bloqueante): carga de datos reales del tenant y corrida del deploy con sus cuentas quedan como operación, no como código pendiente. Ver archivo. |
| Fase 14 — Post-MVP | EN_PROGRESO | 2026-07-24 | Se ejecuta ítem por ítem (ver `fase-14-post-mvp.md`). Hechos: panel PLATFORM_ADMIN (#5), roles/multi-grupo, multi-grupo con misma cuenta, confirmación de obligatorias (#8). En curso: **equipos de trabajo (#9)**. Specs listas de #9 y #10. |
